...
begin
	...
	m05_axis_tlast	<= m_axis_tlast_int;
	m05_axis_tdata	<= data_sr(data_sr'high);

	with state select s05_axis_tready <=
		'0' when IDLE,
		'1' when WAIT_HEADER,
		'1' when RECEIVE_DATA,
		'1' when WAIT_FOOTER,
	...
	with state select m05_axis_tvalid <=
		'0' when IDLE,
		'0' when WAIT_HEADER,
		'0' when RECEIVE_DATA,
		'0' when WAIT_FOOTER,
	...
	begin
		...
		if aresetn = '0' then
			...
			case state is
				...
				when WAIT_HEADER =>
					if s05_axis_tvalid = '1' then
						if s05_axis_tdata = HEADER then
							rx_bytes_counter	<= 0;
							state				<= RECEIVE_DATA;
						else
				...
				when RECEIVE_DATA =>
					if s05_axis_tvalid = '1' then
						...
						if rx_bytes_counter mod 2 = 0 then
							new_word	:= (15 downto 8 => '-') & s05_axis_tdata;
							data_sr		<= new_word & data_sr(0 to data_sr'high-1);
						else
							data_sr(0)(15 downto 8)	<= s05_axis_tdata;
						end if;
